...
# help:
# help: portfoliovisualizerapi Makefile help
# help:
...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt portfoliovisualizerapi
...
.PHONY: format
format:
	@black src/portfoliovisualizerapi tests examples


# help: check-format                   - check code format compliance
.PHONY: check-format
check-format:
	@black --check src/portfoliovisualizerapi tests examples
...
.PHONY: check-types
check-types:
	@cd src; mypy -p portfoliovisualizerapi --ignore-missing-imports


# help: check-lint                     - run static analysis checks
.PHONY: check-lint
check-lint:
	@pylint --rcfile=.pylintrc portfoliovisualizerapi ./tests setup.py ./examples
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/portfoliovisualizerapi*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./portfoliovisualizerapi-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/portfoliovisualizerapi-*-py3-none-any.whl
